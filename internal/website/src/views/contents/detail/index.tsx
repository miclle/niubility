...
// ContentDetail displays a single content item with layout determined by content type.
    ...
    <div className="hidden xl:block flex-shrink-0 w-[400px]">
      ...
      <div className="space-y-3">
        {relatedContents.map((item) => (
          <Link key={item.id} to={`/contents/${item.id}`} className="flex gap-2 no-underline group" style={{ color: 'inherit' }}>
    ...
    return (
      <div className="flex gap-6 p-6 justify-center">
        <div style={{ width: 'max(640px, min(calc((100vh - 180px) * 16 / 9), calc(100vw - 48px)))' }}>
          ...
          {/* Gallery description */}
          {content.summary && (
            <div className="mt-4 p-3 rounded-xl text-sm whitespace-pre-wrap" style={{ background: 'rgba(0,0,0,0.03)', color: '#0f0f0f' }}>
              <div className="flex items-center gap-2 mb-2 text-xs" style={{ color: '#606060' }}>
                <span>{categoryLabel}</span>
                {content.tags?.length > 0 && (
                  <>
