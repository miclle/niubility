...
sequenceDiagram
    ...
    Server->>IdP: 重定向到 IdP
    IdP->>Server: 认证完成后回调 Niubility 服务
...
- **更利于复用现有 Web SSO 能力**：服务端可以沿用或扩展现有 `/sso/callback`、`/sso/acs` 一侧的逻辑
...
func SSOLogin(ctx context.Context, serverURL string) (*Session, error) {
    ...
    // 4. 创建 CLI 登录会话并打开浏览器
    loginSession, err := startSSOLogin(ctx, serverURL, callbackURL)
    if err != nil {
        return nil, fmt.Errorf("start SSO login: %w", err)
    }
    ...
    case result := <-resultChan:
        ...
        // 6. 用 ticket 交换会话
        session, err := exchangeTicket(ctx, serverURL, result.Ticket)
        if err != nil {
            return nil, fmt.Errorf("exchange ticket: %w", err)
        }
