...
1. `AGENTS.md`
2. 与当前任务直接相关的 `.agents/rules/*.md`

## 任务入口

- 后端接口、路由、handler 改动：`.agents/rules/backend-handler.md`
- 业务逻辑、事务、计数、跨实体操作：`.agents/rules/backend-service.md`
- 复杂查询、ORM 与 SQL 取舍：`.agents/rules/query-and-sql.md`
- 设置项、SSO、S3、企业微信、资源分发、数据库备份：`.agents/rules/settings-and-integrations.md`
...
- 遵循既有分层与目录结构，不为局部问题引入新框架、新分层或新状态管理
- 涉及敏感配置时，禁止在日志、文档、测试数据中泄露 secret、token、私钥、证书正文
- 涉及 `podcast`、数据库备份、服务节点、SSO、上传与分发链路时，优先核对前后端与文档联动
- 未完成对应验证前，不要宣称“已修复”“已完成”或“测试通过”
