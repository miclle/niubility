...
// ProfileSettings allows users to edit their own profile.
function ProfileSettings() {
  ...
  return (
    <div className="mx-auto py-8 px-6" style={{ maxWidth: 640 }}>
      <h1 className="text-2xl font-bold mb-6" style={{ color: '#0f0f0f' }}>个人设置</h1>

      <div className="space-y-8">
        {/* Avatar section */}
        <div>
          <Label className="text-sm font-medium mb-3 block" style={{ color: '#0f0f0f' }}>头像</Label>
          <div className="flex items-center gap-4">
            <div className="relative group cursor-pointer" onClick={() => avatarInputRef.current?.click()}>
              <Avatar style={{ width: 96, height: 96 }}>
                <AvatarImage src={avatarURL} alt={name} />
                <AvatarFallback style={{ fontSize: 32 }}>{name?.charAt(0) || currentUser?.username?.charAt(0) || '?'}</AvatarFallback>
              </Avatar>
              <div
            ...
            <span className="text-sm" style={{ color: '#606060' }}>点击头像更换</span>
          </div>
        </div>

        {/* Basic info section */}
        <div className="space-y-4">
          <h2 className="text-base font-medium" style={{ color: '#0f0f0f' }}>基本信息</h2>
